Graphics
========
The ``Graphics`` provides a base class for viewable objects as well as a minimal
tool to visualize AFEM shapes, geometry, and meshes. The most used tool will be
the ``Viewer`` class which can be imported by::
