...
def build_wingbox(wing, params):
    ...
    # STRINGERS ---------------------------------------------------------------
    # Get the shells of the skin and find upper by finding shell with
    # largest z-coordinate.
    shells = ShapeTools.get_shells(skin)
    shell = shells[0]
    zmax = ShapeTools.center_of_mass(shell).z
    if len(shells) > 1:
        for s in shells[1:]:
            zi = ShapeTools.center_of_mass(s).z
            if zi > zmax:
                shell = s
                zmax = zi

    # Define circles of increasing radius at root of root rib to generate
    # curved stringers.
    line = CreateGeom.line_by_points(root.p1, root.p2)
    p0 = line.eval(3.5 * root.cref.length)
    ...
    indx = 1
    for i in range(20):
        geom_circle = GC_MakeCircle(p0, vn, r0).Value().GetObject()
        r0 *= 1.025
