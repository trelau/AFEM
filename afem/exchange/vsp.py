...
class ImportVSP(object):
    ...
    @classmethod
    def step_file(cls, fn, divide_closed=True, bspline_restrict=False,
                  tol=0.01, reloft=False):
        """
        ...
        :param bool bspline_restrict: Option to attempt to refit the
            underlying OpenVSP surfaces with surfaces of higher continuity.
            This may take a long time to run and no guarantee it will be
            successful. At the moment this is only used on known wing
            components. This method is experimental.
        ...
        while iterator.More() and more:
            ...
            if metadata['m_Type'] == 5 and metadata['m_SurfType'] != 99:
                wing = _process_wing(compound, divide_closed, bspline_restrict,
                                     tol, reloft)
                if wing is not None:
    ...
    def rebuild_wing_solid(srfs, divide_closed=True):
        ...
        nsrfs = len(srfs)
        if nsrfs == 1:
            return _process_unsplit_wing(compound, divide_closed, False, 0.)
        elif nsrfs > 1:
...
def _bspline_restrict(solid, tol):
    ...
    logger.info('\tAttempting to refit the surface...')
    tool = ShapeBSplineRestriction(solid, dmax=1, tol3d=tol)
    if not tool.is_done:
        logger.info('Refitting unsuccessful. Using original solid.')
        return solid
    ...
    logger.info('\tRefitting successful with surface error: {}'.format(
        tool.error_surface))
...
def _topods_iterator_check(shape, check):
    ...
    from OCCT.BRepCheck import BRepCheck_NoError
    ...
    while it.More():
        ...
        los = result.Status()
        for stat in los:
            if stat != BRepCheck_NoError:
                print(stat, s.ShapeType())
        ...
        it.Next()
        _topods_iterator_check(s, check)
